...
for simulation_number in range(10):
    ...
    #packet reception at relays
            for rcvr_id in range(numGateways, nrBS):
                ...
                if gpsFlag == 1:
                    # print("GPS is ON")
                    if bs[rcvr_id].dist > node.packet[rcvr_id].dist :
                        flagg = 1
...
power_consumed=open("powerConsumedRelayDistanceLessSimsWithGPSWithoutRelayCircleCorrectedDup15.txt","a")
power_consumed.write(str(powerConsumed/10))
power_consumed.write(',')
power_consumed.close()

fdrConsumed=open("fdrRateRelayDistanceLessSimsWithGPSWithoutRelayCircleCorrectedDup15.txt","a")
