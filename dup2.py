...
# specify simulation parameters
    num_nodes_range = [1]; # number of nodes in the network
    ...
    relays = 0 # number of relay nodes
    ...
    circ_rad = 1000 # radius of the circle over which the ndoes are randomly distributed (gateway-0 is at the center)
    n0_dist = x # distance between node-0 and gateway-0
    ...
    relay_radius = 0 # radius of the circle over which the relays are randomly distributed
    ...
    for num_nodes in num_nodes_range:
        for weak_interference in range(0,1,1):
            ...
            import sys;
            sys.argv=[r'LoRa_Relay_v2_small.py', str(num_nodes),str(avg_inter_frame_interval),str(exp_num),str(simulation_time),\
                        ...
                        str(relay_reception_time), str(relay_transmission_time), str(relay_sleep_time)];
            exec(open(r'LoRa_Relay_v2_small.py').read());
