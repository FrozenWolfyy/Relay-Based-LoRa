...
redunancy = 0 # ignore this parameter
...
circ_rad = 150 # radius of the circle over which the ndoes are randomly distributed (gateway-0 is at the center)
n0_dist = 125 # distance between node-0 and gateway-0
...
path_loss_exponent = 2 # determines signal attenuation as it travels from sender to receiver
weak_interference = 1 # set to 1 if a weak frame can cause sync. failure for a strong frame
relay_radius = 50 # radius of the circle over which the relays are randomly distributed
...
for num_nodes in num_nodes_range:
    for weak_interference in range(0,1,1):
        for i in range(simulation_runs):
            ...
            class myNode():
                def __init__(self, id, period):
                    ...
                    if self.id == 0:
                        ...
                        self.dist = random.uniform(0, circle_radius)
            ...
            #
            class myPacket():
                def __init__(self, nodeid, plen, distance, rcvr_id):
                    global experiment
                    global Ptx
                    global gamma
                    global d0
                    ...
                    propagation_loss = Lpld0 + 10*gamma*math.log10(distance/d0)
            ...
            def transmit(env,node):
                while True:
                    ...
                    for rcvr_id in range(0, nrBS):
                        ...
                        node.packet.append(myPacket(node.id, payload_size, dist, rcvr_id))
                    ...
                    for rcvr_id in range(numGateways, numRelays + numGateways):
...
print(total_fdr/simulation_runs)
