...
# specify simulation parameters
    num_nodes_range = [random.randint(10,250)] # number of nodes in the network
    ...
    relays = random.randint(0,15) # number # of relay nodes
    ...
    circ_rad = random.randint(20,250) # radius of the circle over which the ndoes are randomly distributed (gateway-0 is at the center)
    n0_dist = 30 # distance between node-0 and gateway-0
    ...
    relay_radius =  random.randint(10,circ_rad)  # radius of the circle over which the relays are randomly distributed
    relay_reception_time = 1000 # in ms
    relay_transmission_time = 1000 # in ms
    relay_sleep_time = 5000 # in ms
    relay_circle=1
    gpsFlag=1
    alpha = ((random.randint(0,150))/100)
    ...
    for num_nodes in num_nodes_range:
        for weak_interference in range(0,1,1):
            for i in range(1):
                import sys;
                sys.argv=[r'LoRa_ztest.py', str(num_nodes),str(avg_inter_frame_interval),str(exp_num),str(simulation_time),\
                ...
                exec(open(r'LoRa_ztest.py').read());
