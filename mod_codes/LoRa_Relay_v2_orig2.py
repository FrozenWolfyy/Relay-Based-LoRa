...
for simulation_number in range(100):
    ...
    class myNode():
        def __init__(self, id, period):
            ...
            if experiment == 0:
                ...
                if dynamic_sensor_sf == 0:
                    self.sf = allocate_SF(self.dist)
                else:
                    flag=self.dist
                    if (self.dist - relay_radius >0 ):
                            flag = self.dist - relay_radius
                    self.sf=allocate_SF(flag)
                # print(self.sf)
    ...
    # nrBs->totol number of basestations
            ...
            for rcvr_id in range(numGateways, numRelays + numGateways):
                ...
                # if (packet.dist > bs[rcvr_id].dist):
                if (node in packetsAtBS[rcvr_id]):
                    print ("ERROR: packet already in")
                else:
                    # adding packet if no collision
    ...
    if len(sys.argv) == 28:
        ...
        relay_circle = int(sys.argv[26]) #set to 1 for placing relays in a circle
        dynamic_sensor_sf=int(sys.argv[27])
    else:
        print(len(sys.argv))
...
# sf_sensors_text=open("sf_sensors.txt","a")
# sf_sensors_text.write(str(avg_fdr_rate))
# sf_sensors_text.write('\n')
# sf_sensors_text.close()
