...
# specify simulation parameters
    ...
    n0_dist = 50 # distance between node-0 and gateway-0
    ...
    weak_interference = 1000 # set to 1 if a weak frame can cause sync. failure for a strong frame
    relay_radius = 75  # radius of the circle over which the relays are randomly distributed
    relay_reception_time = 1000 # in ms
    relay_transmission_time = 0 # in ms
    ...
    for num_nodes in num_nodes_range:
        for weak_interference in range(0,1,1):
            ...
            import sys;
            sys.argv=[r'LoRa_Relay_v2_dup13.py', str(num_nodes),str(avg_inter_frame_interval),str(exp_num),str(simulation_time),\
            ...
            exec(open(r'LoRa_Relay_v2_dup13.py').read());